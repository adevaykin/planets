...
impl AllocatedBuffer {
    pub(super) fn new_with_size(device: &mut Device, size: u64, usage: vk::BufferUsageFlags, mem_props: vk::MemoryPropertyFlags) -> AllocatedBuffer {
        ...
        unsafe { device.logical_device.bind_buffer_memory(buffer, memory, 0).expect("Failed to bind buffer memory"); }

        AllocatedBuffer { is_allocated: true, buffer, memory, size }
    }

    pub(super) fn new_with_staging(device: &mut Device, data: &impl BufferData, usage: vk::BufferUsageFlags) -> AllocatedBuffer {
        ...
        unsafe { device.logical_device.bind_buffer_memory(staging_buffer, staging_memory, 0).expect("Failed to bind memory"); }
        ...
        unsafe {
            device.logical_device.bind_buffer_memory(buffer, memory, 0).expect("Failed to bind buffer memory");
        }
        ...
        AllocatedBuffer { is_allocated: true, buffer, memory, size: data.size() as u64 }
    }

    pub(super) fn new_host_visible_coherent(device: &mut Device, data: &impl BufferData, usage: vk::BufferUsageFlags) -> AllocatedBuffer {
        let (buffer, memory) = AllocatedBuffer::create_buffer(
            device,
            data.size() as u64,
            usage,
    ...
    fn update_data_intern(device: &Device, memory: vk::DeviceMemory, data: &impl BufferData, offset: u64) {
        unsafe {
            let mapped_memory = device.logical_device.map_memory(memory, offset, data.size() as u64, vk::MemoryMapFlags::empty()).expect("Failed to map memory") as *mut u8;
            mapped_memory.copy_from_nonoverlapping(data.as_ptr(), data.size());
            device.logical_device.unmap_memory(memory);
        }
    }

    fn create_buffer(device: &Device, size: u64, usage: vk::BufferUsageFlags, properties: vk::MemoryPropertyFlags) -> (vk::Buffer, vk::DeviceMemory) {
        ...
        let buffer = unsafe { device.logical_device.create_buffer(&create_info, None).expect("Failed to create buffer") };
        ...
        let allocate_info = vk::MemoryAllocateInfo {
            ...
            memory_type_index: device.find_memory_type(mem_requirements.memory_type_bits, properties),
            ..Default::default()
        };

        let memory = unsafe { device.logical_device.allocate_memory(&allocate_info, None).expect("Failed to allocate memory") };
    ...
    fn copy_buffer(device: &Device, src: vk::Buffer, dst: vk::Buffer, size: vk::DeviceSize) {
        ...
        unsafe {
            device.logical_device.cmd_copy_buffer(single_time_cmd_buffer.get_cmd_buffer(), src, dst, &copy_regions);
        }
    }
}
...
mod tests {
    ...
    fn empty_vec_buffer_data() {
        ...
        assert_eq!(test_vec_buffer_data.stride(), std::mem::size_of::<u8>() as u32);
    ...
    fn f32_vec_buffer_data() {
        ...
        assert_eq!(test_vec_buffer_data.as_ptr(), test_vec.as_ptr() as *const u8);
    }
    ...
    fn struct_buffer_data() {
        let test_data = TestData{ value: 42, value2: 42.0 };
        let test_data_buffer = StructBufferData::new(&test_data);
        assert_eq!(test_data_buffer.size(), std::mem::size_of::<i32>() + std::mem::size_of::<f32>());
