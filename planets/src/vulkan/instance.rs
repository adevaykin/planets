...
impl VulkanInstance {
    pub fn new(entry: &ash::Entry) -> VulkanInstance {
        ...
        let (debug_utils_loader, debug_messenger) = VulkanInstance::setup_debug_callback(&entry, &instance);
        VulkanInstance { instance, debug_utils_loader, debug_messenger }
    }
    ...
    fn create_instance(entry: &ash::Entry) -> ash::Instance {
        ...
        let create_info = vk::InstanceCreateInfo {
            ...
            p_next: if helpers::is_debug() {
                    &debug_messenger_create_info as *const vk::DebugUtilsMessengerCreateInfoEXT as *const c_void
                } else {
                    ptr::null()
                },
    ...
    fn check_validation_layers_support(entry: &ash::Entry) -> bool {
        ...
        let available_layers = entry.enumerate_instance_layer_properties().expect("Unable to enumerate validation layers");
        for required_layer_name in &required_layers {
            ...
            if !found {
                println!("Didn't find required validation layer: {}", required_layer_name);
                return false;
            }
        }
    ...
    -> (Option<ash::extensions::ext::DebugUtils>, Option<vk::DebugUtilsMessengerEXT>) {
    ...
    fn setup_debug_callback(_entry: &ash::Entry, _instance: &ash::Instance)
    -> (Option<ash::extensions::ext::DebugUtils>, Option<vk::DebugUtilsMessengerEXT>) {
        (None, None)
    }
}
    ...
    fn drop(&mut self) {
        unsafe {
            if self.debug_utils_loader.is_some() {
                self.debug_utils_loader.as_ref().unwrap().destroy_debug_utils_messenger(self.debug_messenger.unwrap(), None);
            }
