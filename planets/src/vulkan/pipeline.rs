...
use super::shader::{ShaderManagerMutRef, Shader};
...
struct ViewportDefinition {
    #[allow(dead_code)] /// TODO: use _ everywhere here instead of allow(dead_code)
...
impl Pipeline {
    ...
    fn create_fragment_stage_info(shader: &Shader) -> ShaderStageDefinition {
        ...
        ShaderStageDefinition { main_func_name, create_info }
    }
    ...
    fn create_viewport_state_def(size_def: &impl ViewportSize) -> ViewportDefinition {
        ...
        let scissors = vec![vk::Rect2D {
            offset: vk::Offset2D { x: size.offset_x as i32, y: size.offset_y as i32 },
            extent: vk::Extent2D { width: size.width as u32, height: size.height as u32 }
        }];
        ...
        ViewportDefinition { viewports, scissors, create_info }
    }
    ...
    fn create_color_blend_attachment_state_def() -> ColorAttachmentDef {
        ...
        ColorAttachmentDef { attachments, create_info }
    }

    fn create_descriptor_set_layout(device: &DeviceMutRef, layout_bindings: &Vec<vk::DescriptorSetLayoutBinding>) -> vk::DescriptorSetLayout {
        ...
        let layout = unsafe { device.borrow().logical_device.create_descriptor_set_layout(&create_info, None).expect("Failed to create descriptor set layout") };

        layout
    }

    fn create_layout(device: &ash::Device, descriptor_set_layout: &vk::DescriptorSetLayout) -> vk::PipelineLayout {
        ...
        let layout = unsafe {
            device.create_pipeline_layout(&create_info, None).expect("Failed to create pipeline layout")
        };
...
impl Drop for Pipeline {
    fn drop(&mut self) {
        unsafe {
            self.device.borrow().logical_device.destroy_descriptor_set_layout(self.descriptor_set_layout, None);
            self.device.borrow().logical_device.destroy_pipeline_layout(self.layout, None);
            for pipeline in &self.pipelines {
                self.device.borrow().logical_device.destroy_pipeline(*pipeline, None);
            }
        }
    }
}
...
impl PipelineAssembly {
    pub fn assamble(&mut self) -> Pipeline {
        ...
        Pipeline { device: Rc::clone(&self.device), pipelines, layout, descriptor_set_layout }
    }

    pub fn with_depth_stencil_info(&mut self, info: vk::PipelineDepthStencilStateCreateInfo) -> &mut PipelineAssembly {
        self.depth_stencil_info = Some(info);
        self
    }

    pub fn with_layout_bindings(&mut self, layout_bindings: Vec<vk::DescriptorSetLayoutBinding>) -> &mut PipelineAssembly {
        self.layout_bindings = layout_bindings;
        self
    }

    pub fn with_vertex_input_binding_description(&mut self, bindings: vk::VertexInputBindingDescription) -> &mut PipelineAssembly {
        self.vertex_input_binding_description = Some(bindings);
        self
    }

    pub fn with_vertex_input_attribute_description(&mut self, attributes: Vec<vk::VertexInputAttributeDescription>) -> &mut PipelineAssembly {
        self.vertex_input_attribute_descriptions = Some(attributes);
        self
    }

    fn create_graphics_pipelines(&mut self) -> (vk::PipelineLayout, vk::DescriptorSetLayout, Vec<vk::Pipeline>) {
        ...
        let stages = vec![
            stages_def[0].create_info,
            stages_def[1].create_info
        ];
        ...
        let depth_stencil_state = match self.depth_stencil_info {
            None => vk::PipelineDepthStencilStateCreateInfo{ ..Default::default() },
            Some(info) => info
        };
        ...
        let descriptor_set_layout = Pipeline::create_descriptor_set_layout(&self.device, &self.layout_bindings);
        let layout = Pipeline::create_layout(&self.device.borrow().logical_device, &descriptor_set_layout);
        ...
        let pipeline = unsafe {
            self.device.borrow().logical_device.create_graphics_pipelines(vk::PipelineCache::null(), &create_info, None)
