...
impl Shader {
    fn new(device: &DeviceMutRef, path: &Path, name: &str) -> Shader {
        ...
        let vertex_data = util::fs::read_bin_file(vertex_path.as_path()).expect("Could not load vertex shader");
        let vertex_module = Shader::create_module(&device.borrow().logical_device, vertex_data);
        debug::Object::label(&device.borrow(), vk::ObjectType::SHADER_MODULE, vertex_module.as_raw(), vert_filename.as_str());

        let mut frag_filename = String::from(name);
        frag_filename.push_str(".frag.spv");
        let fragment_path = path.join(Path::new(&frag_filename));
        let fragment_data = util::fs::read_bin_file(fragment_path.as_path()).expect("Could not load fragment shader");
        let fragment_module = Shader::create_module(&device.borrow().logical_device, fragment_data);
        debug::Object::label(&device.borrow(), vk::ObjectType::SHADER_MODULE, fragment_module.as_raw(), frag_filename.as_str());

        Shader { device: Rc::clone(&device), vertex_module: Some(vertex_module), fragment_module: Some(fragment_module) }
    }
    ...
    fn create_module(device: &ash::Device, data: Vec<u8>) -> vk::ShaderModule {
        ...
        let module = unsafe { device.create_shader_module(&shader_module_info, None).expect("Failed to create shader module") };
    ...
    fn drop(&mut self) {
        unsafe {
            if self.vertex_module.is_some() {
                self.device.borrow().logical_device.destroy_shader_module(self.vertex_module.expect("Failed to destroy shader module"), None);
            }
            if self.fragment_module.is_some() {
                self.device.borrow().logical_device.destroy_shader_module(self.fragment_module.expect("Failed to destroy shader module"), None);
            }
        }
    }
}