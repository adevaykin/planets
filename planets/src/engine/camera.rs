...
pub const UP: cgm::Vector3<f32> = cgm::Vector3{ x: 0.0, y: 1.0, z: 0.0 }; // TODO: move this constant to some kind of World from Camera
...
impl Camera {
    pub fn new(resource_manager: &mut ResourceManager) -> Camera {
        let position = cgm::Point3 { x: 0.0, y: 0.0, z: -10.0 };
        let up = UP;
        let aspect = 4.0 / 3.0;
        let mut ubo_interface = CameraUBOInterface {
            ...
            viewport_extent: cgm::Vector4{ x: 0.0, y: 0.0, z: 0.0, w: 0.0 },
        };
        ...
        Camera { position, up, aspect, ubo_interface, ubos }
    }
    ...
    pub fn update(&mut self, device: &Device, frame_num: usize, viewport_size: &dyn ViewportSize) {
        let mut ubo_interface = CameraUBOInterface {
            ...
            viewport_extent: cgm::Vector4{ x: viewport_size.get_size().offset_x, y: viewport_size.get_size().offset_y, z: viewport_size.get_size().width, w: viewport_size.get_size().height },
        };
        ubo_interface.proj[1][1] *= -1.0;
