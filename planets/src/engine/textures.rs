...
impl TextureManager {
    pub fn new(device: &DeviceMutRef, resource_manager: &ResourceManagerMutRef) -> TextureManager {
        let invalid_image = Rc::new(Image::from_file(device, &mut resource_manager.borrow_mut(), INVALID_IMAGE_PATH).unwrap());
        let mut loaded: HashMap<String, Rc<Image>> = HashMap::new();
        loaded.insert(INVALID_IMAGE_PATH.to_string(), invalid_image);

        TextureManager { device: Rc::clone(device), resource_manager: Rc::clone(resource_manager), loaded }
    }
    ...
    pub fn get_texture(&mut self, path: &str) -> Rc<Image> {
        ...
        if existing.is_some() {
            ...
            let new_image = Image::from_file(&self.device, &mut self.resource_manager.borrow_mut(), path);
            if new_image.is_ok() {
                self.loaded.insert(path.to_string(), Rc::new(new_image.unwrap()));
