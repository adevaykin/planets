...
impl Node {
    ...
    pub fn remove_child(&mut self, child_to_remove: &NodeMutRef) {
        self.children.retain(|child| !Node::children_equal(child, child_to_remove));
    ...
    pub fn update(&mut self, device: &Device, frame_num: usize, timer: &TimerMutRef, light_manager: &LightManagerMutRef, transform: &cgm::Matrix4<f32>) {
        ...
        for child in &mut self.children {
            ...
            child.borrow_mut().update(device, frame_num, timer, light_manager, &next_transform);
        }
